...
$(function(){
    ...
    (function progress(){
        ...
        $("#progress_time").on("click",function(e){
            updProgress(e.pageX,player,e);
        });
        $("#volume").on("click",function(e){
            updVolume(e.pageX,player);
        });
    })();
    //dblclick chang the song
    $("tbody").on("dblclick","tr",function(){
       $("#musicPlayer")[0].src =  $(this).attr("value");
        //console.log($(this).index());
        upInfo($(this).index())
        now = parseInt($(this).find("td").eq(0).text())-1;
        rest();
        player.play();
    ...
    $(".type").on("click",function(){
       switch ($(this).find("i").attr("class")){
           case "glyphicon glyphicon-random":
               $(this).find("i").attr("class","glyphicon glyphicon-retweet");
               break;
           case "glyphicon glyphicon-retweet":
               $(this).find("i").attr("class","glyphicon glyphicon-random");
               break;
       }
    });
    //next song  when click btn 'next'
    $(".next").on("click",function(){
        next();
    });
    //prev song  when click btn 'prev'
    $(".prev").on("click",function(){
        prev();
    });
});
//update progress when clicked
function updProgress(x,player,event){
    console.log(x);
...
//update name and singer
function upInfo(now){
    $(".info_detail p").eq(0).text($("tbody").find("tr").eq(now).find("td").eq(1).text());
    $(".info_detail p small").eq(1).text($("tbody").find("tr").eq(now).find("td").eq(2).text());
}
//next song
function next(){
    var player = $("#musicPlayer")[0];
    var type = $(".type").find("i").attr("class");
    var  type1=type=="glyphicon glyphicon-retweet"?"retweet":"random";
    console.log(type1);
    if(type1 == "retweet"){
        if(now+1<$("tbody").find("tr").length){
            now = now+1;
        }else{
            now = 0;
        }
        player.src = $("tbody").find("tr").eq(now).attr("value");
        player.play();
        upInfo(now);
    }
    if(type1 == "random"){
        var num = Math.round(Math.random()*($("tbody").find("tr").length-1));
        console.log(num);
        player.src = $("tbody").find("tr").eq(num).attr("value");
        player.play();
        upInfo(num);
        now = num;
    }
